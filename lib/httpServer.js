...
function authenticateRequest(_request, _errorCallback, _successCallback) {
    ...
    Repository.Client.findByName(_request.headers[authHeaderClientName], function(_error, _client) {
        if (!_error) {
            if (_client) {
                _request.logger = _request.logger.child({ client: _client.name });
                var validHash = Crypto.createHash('sha1')
                    .update(_client.privateKey)
                    .update(_request.url)
                    .digest('hex');

                if (validHash != _request.headers[authHeaderSecurityHash]) {
                ...
                return _errorCallback({
                    client: _request.headers[authHeaderClientName],
